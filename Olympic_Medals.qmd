...
They now need to win `r fitted_model$slope[[1]] %>% round(1)` medals on average per day to match their `r fitted_model %>% filter(year == selected_year[[1]]) %>% pull(max_running_total_year_country) %>% round(1)` medals from `r selected_year[[1]]`.
